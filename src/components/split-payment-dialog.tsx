...
export function SplitPaymentDialog({
  ...
  function submit(formData: FormData) {
    startTransition(async () => {
      const result = await saveTransactionSplit(formData).catch((): ActionState => ({ status: 'error', message: 'saveSplitFailed' }))
      setState(result)
  ...
  return (
    <>
      <button className="ledger-button" disabled={isPending} onClick={() => {
        if (split) {
          unsplit()
          return
        }
        setSplitCount(2)
        setPersonalAmount(dollars(defaultPersonalShareCents(amountCents, 2)))
        setState({ status: 'idle', message: '' })
        dialog.current?.showModal()
      }} type="button">{split ? dictionary.unsplit : dictionary.split}</button>
      {split && state.status === 'error' && <p className="split-error" role="alert">{dictionary[state.message as 'invalidSplit' | 'saveSplitFailed']}</p>}
