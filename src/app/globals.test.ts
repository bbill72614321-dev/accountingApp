...
describe('mobile split payment dialog', () => {
  it('keeps split fields readable without widening or zooming the viewport', () => {
    ...
    expect(mobileStyles).toContain('.split-actions { justify-content: flex-start; margin-top: 0.5rem; width: 100%; }')
    expect(mobileStyles).toContain('.split-actions .button { flex: 1 1 0; min-height: 2.75rem; }')
  })
})
